...
export const StubPage = ({ title, subtitle, icon: Icon }: { title: string; subtitle: string; icon: LucideIcon }) => (
  <AppLayout>
    <div className="px-4 lg:px-8 py-8 min-h-[60vh] flex flex-col items-center justify-center text-center animate-fade-up">
      ...
      <div className="mt-8 grid grid-cols-1 sm:grid-cols-3 gap-3 w-full max-w-2xl">
        {Array.from({ length: 3 }).map((_, i) => (
          <div key={i} className="gradient-card border border-border rounded-xl h-24 shimmer opacity-30" />
        ))}
      </div>
    </div>
  </AppLayout>
