...
describe("TournamentTabs", () => {
  const tabs: TournamentTabKey[] = ["overview", "teams", "lobbies", "rules"];
  ...
  it.each(tabs)("highlights %s as active tab", (tab) => {
    const onChange = vi.fn();
    renderWithProviders(<TournamentTabs activeTab={tab} onChange={onChange} />);
  ...
  it("calls onChange when lobbies tab is clicked", async () => {
    ...
    await userEvent.click(screen.getByRole("button", { name: /lobbies/i }));
...
describe("TournamentHero", () => {
  it("renders tournament title in uppercase", () => {
    ...
    renderWithProviders(<TournamentHero tournament={t} activeTab="overview" />);
  ...
  it("renders 'Next match in' text when on lobbies tab", () => {
    const t = makeTournament();
    renderWithProviders(<TournamentHero tournament={t} activeTab="lobbies" />);
  ...
  it("renders banner image", () => {
    ...
    renderWithProviders(<TournamentHero tournament={t} activeTab="overview" />);
    expect(screen.getByRole("img", { name: "Hero Test" })).toBeInTheDocument();
  });
...
describe("OverviewTab", () => {
  ...
  it("renders tournament start date", () => {
    ...
    renderWithProviders(<OverviewTab tournament={t} />);
    expect(screen.getByText("Tue 24th Jan 2024, 01:00 PM")).toBeInTheDocument();
  });
  ...
  it("renders format and check-in details", () => {
    ...
    renderWithProviders(<OverviewTab tournament={t} />);
    expect(screen.getAllByText("Single Elimination")[0]).toBeInTheDocument();
    expect(
  ...
  it("renders how-to-join steps", () => {
    ...
    renderWithProviders(<OverviewTab tournament={t} />);
    expect(screen.getByText("Step one instructions")).toBeInTheDocument();
    expect(screen.getByText("Step two instructions")).toBeInTheDocument();
  });
  ...
  it("renders organiser name", () => {
    ...
    expect(screen.getAllByText("GS ESPORTS").length).toBeGreaterThan(0);
  });
  ...
  it("handles tournament with no prizes gracefully", () => {
    ...
    expect(screen.getByText(/Total Tournament Prize/i)).toBeInTheDocument();
  });

  it("handles tournament with no rounds gracefully", () => {
    const t = makeTournament({ rounds: [] });
    renderWithProviders(<OverviewTab tournament={t} />);
    expect(screen.getByText(/Rounds and Schedule/i)).toBeInTheDocument();
  });
});
...
describe("LobbiesTab", () => {
  ...
  it("renders round filter buttons", () => {
    ...
    const onChange = vi.fn();
    renderWithProviders(
      <LobbiesTab tournament={t} round={1} onRoundChange={onChange} />,
    );
    expect(
  ...
  it("shows only lobbies for the selected round", () => {
    ...
    const onChange = vi.fn();
    renderWithProviders(
      <LobbiesTab tournament={t} round={2} onRoundChange={onChange} />,
    );
  ...
  it("calls onRoundChange with correct round number when filter clicked", async () => {
    ...
    await userEvent.click(screen.getByRole("button", { name: /round 3/i }));
  ...
  it("renders empty list when no lobbies exist for selected round", () => {
    ...
    expect(screen.queryByText(/View Participants/i)).not.toBeInTheDocument();
  });
...
describe("RulesTab", () => {
  it("renders each rule as a list item", () => {
    ...
    expect(screen.getByText(/No teaming with players/)).toBeInTheDocument();
    expect(
